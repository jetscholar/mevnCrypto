...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (!bucketListItem) throw new Error('Fubar with the save op, G!')
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        if (!response) throw Error('Something went wrong, G!')
...
router.delete('/:id', async (req, res) => {
    const { id } = req.params
    try {
        const removed = await BucketListItem.findIdAndDelete(id)
